...
public class Validator {
    ...
    private var callBacks = ValidatorDictionary<(String) -> Void>()
    ...
    private func validateAllFields() {
        ...
        for (_, rule) in validations {
            let callback = callBacks[rule.field]
    ...
    public func registerField(_ field: ValidatableField, errorLabel: UILabel? = nil, rules: [Rule], callBack: @escaping ((String) -> Void)) {
    ...
    func registerInputView(_ inputView: InputFieldView, rules: [Rule]) {
        ...
        registerField(txtField!, errorLabel: inputView.errorLabel, rules: rules) { error in
            ...
            if error.isEmpty {
                ...
                inputView.contentTextField.borderWidth = 1
                inputView.errorLabel.text = "(\(error))"
            }
        }
    }
    ...
    public func validate(_ delegate: ValidationDelegate) {
        ...
        if errors.isEmpty {
            ...
            for error in errors {
                let field = error.value.field
                if let callback = callBacks[field] {
                    callback(error.value.errorMessage)
                }
            }
